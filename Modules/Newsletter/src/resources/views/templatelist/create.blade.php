                ...
                <div class="col-lg-12">
                    <div class="card border-0 rounded shadow ">
                        <div class="modal-body ">
                            <form action="{{ route('template-list.store') }}" id="userForm" method="POST" class="needs-validation" novalidate>
                                @csrf
                                <div class="card-header bg-transparent py-2">
                                    <h5 class="text-md-start text-center mb-0">{{__('newsletter.add_template_list')}}</h5>
                                </div>
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="mb-3">
                                            ...
                                            <div class="form-icon position-relative">
                                                <input name="subject" id="add_subject" type="text" class="form-control" placeholder="{{ __('newsletter.subject_placeholder')}}" required>
                                            ...
                                            <label class="form-label">{{ __('newsletter.group_list')}}
                                                ...
                                                <select class="form-select form-control" id="group_list" name="group_list"
                                                    ...
                                                    @foreach ($template_group_list as $group_list)
                                                        <option value="{{ $group_list->id }}" {{ session()->get('temlategroup') ==  $group_list->id ? "selected" : ''  }}> {{ $group_list->group_name }}</option>
                                                    @endforeach
                                                    @endif
                                                </select>
                                                <span style="color:red;">
