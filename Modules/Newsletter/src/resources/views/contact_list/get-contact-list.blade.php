                    ...
                    <div class="table-responsive shadow rounded" id="website_table">
                        <div class=" border-0 quotation_form">
                            <div class="card-header py-3 bg-transparent d-flex align-items-center justify-content-between">
                                <h5 class="tx-uppercase tx-semibold mg-b-0" id="">@if(!empty($showGroupName[0]->group_name)){{$showGroupName[0]->group_name}}@endif</h5>

                                <div>
                                    <a href="{{ route('contact-group.index') }}">
                                        <button class="btn btn-primary ">{{__('common.goback')}}</button></a>
                                </div>
                            </div>
                        </div>
                        @if(!empty($ShowContact) && sizeof($ShowContact)>0)
                        <table class="table table-center bg-white mb-0">
                            ...
                            <tbody>
                                ...
                                @foreach ($ShowContact as $key => $contact)
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-check form-switch mx-auto pl-0 ps-5">
                                          <input id="loader" data-id="{{ $contact->id }}" class="form-check-input mx-auto toggle_class" type="checkbox" data-toggle="toggle" data-on="Active" data-off="InActive" {{ $contact->status ? 'checked' : '' }}>
                                         </div>
                                    </td>
        ...
        // change status in ajax code start
            $('.toggle_class').change(function(e) {
                ...
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "{{route('ChangeContactToGroupStatus')}}",
                    data: {
                        'status': status,
                        'id': id
                    },
