    ...
    <div class="container-fluid mt-3">
        <div class="container">
            <nav class="breadcrumb bg-transparent m-0 p-0">
                ...
                <span class="breadcrumb-item active">HTML</span>
            </nav>
        </div>
    </div>
    <!-- Breadcrumb End -->
                            ...
                            @foreach ($bloglist as $key => $blog)
                                ...
                                <div class="col-lg-6">
                                    <div class="d-flex mb-3 service-item first-service">
                                        <a href="{{ route('blog.details', ['category' => $blog->category_slug, 'slug' => $blog->slug]) }}"><img src="{{ asset($blog->image) }}"
                                        ...
                                        <div class="w-100 d-flex flex-column justify-content-center px-3"
                                            ...
                                            <a class="h6 m-0"
                                                href="{{ route('blog.details', ['category' => $blog->category_slug, 'slug' => $blog->slug]) }}">{{ $blog->title }}</a>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div class="pb-3">
                        ...
                        <div class="bg-light p-4 mb-3">
                            <form action="{{ route('user.comment.store') }}" method="POST"
                                ...
                                @csrf
                                <input type="hidden" name="category" value="html" required>
                                {{-- <div class="col-md-12 mb-3">
                                    <label for="validationCustom01" class="form-label text-danger fw-2">Category</label>
                                    <input type="text" readonly name="category" class="form-control"
                                        id="validationCustom01" value="html" required>
                            ...
                            @if (!empty($categorys))
                                @foreach ($categorys as $keys => $category)
                                    <a href="{{ asset('user' . '/' . strtolower($category->slug)) }}"
                                        class="btn btn-sm btn-outline-secondary m-1 {{ $category->slug == 'html' ? 'bg-danger text-light' : '' }}">
