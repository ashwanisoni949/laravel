    ...
    <div class="container-fluid mt-3">
        <div class="container">
            <nav class="breadcrumb bg-transparent m-0 p-0">
                ...
                <span class="breadcrumb-item active">CI</span>
            </nav>
        </div>
    </div>
    <!-- Breadcrumb End -->
                    ...
                    <div class="pb-3">
                        ...
                        <div class="bg-light p-4 mb-3">
                            <form action="{{ route('user.comment.store') }}" method="POST"
                                ...
                                @csrf
                                <input type="hidden" name="category" value="ci" required>
                            ...
                            @if (!empty($categorys))
                                @foreach ($categorys as $keys => $category)
                                    <a href="{{ asset('user' . '/' . strtolower($category->category_name)) }}"
                                        class="btn btn-sm btn-outline-secondary m-1 {{ $category->category_name == 'CI' ? 'bg-danger text-light' : '' }}">
