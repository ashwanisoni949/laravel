    ...
    <div class="container-fluid mt-3">
        <div class="container">
            <nav class="breadcrumb bg-transparent m-0 p-0">
                ...
                <span class="breadcrumb-item active">Computer</span>
            </nav>
        </div>
    </div>
    <!-- Breadcrumb End -->
                    ...
                    <div class="pb-3">
                        ...
                        <div class="bg-light p-4 mb-3">
                            <form action="{{ route('user.comment.store') }}" method="POST"
                                ...
                                @csrf
                                <input type="hidden" name="category" value="computer" required>
                            ...
                            @if (!empty($categorys))
                                @foreach ($categorys as $keys => $category)
                                    <a href="{{ asset('user' . '/' . strtolower($category->category_name)) }}"
                                        class="btn btn-sm btn-outline-secondary m-1 {{ $category->category_name == 'Computer' ? 'bg-danger text-light' : '' }}">
    ...
    <script>
        $(document).ready(function() {
            $("#searchbar").on("keyup", function() {
                ...
                const url = "{{ route('user.computer-search') }}";
                $.ajaxSetup({
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        var path = '{{ URL::asset('/') }}';
                        var html = '';
                        if (response.computerlist != '') {
                            $.each(response.computerlist, function(key, blog) {
