        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-6 py-3">
                    ...
                    <div class="owl-carousel owl-carousel-3 carousel-item-2 position-relative">
                        @if (!empty($totalblog))
                            @foreach ($totalblog->where('category_slug', 'course') as $keys => $result)
                                @php
                ...
                <div class="col-lg-6 py-3">
                    <div class="bg-light py-2 px-4 mb-3 border border-dark">
                        <h3 class="m-0">10-12th UP Board</h3>
                    </div>
                    <div class="owl-carousel owl-carousel-3 carousel-item-2 position-relative">
                        @if (!empty($totalblog))
                            @foreach ($totalblog->where('category_slug', '10th-12th-up-board') as $keys => $laravel)
                                @php
     ...
     <div class="container-fluid">
        <div class="container">
            <div class="row">
                <div class="col-lg-6 py-3">
                    <div class="bg-light py-2 px-4 mb-3 border border-dark">
                        <h3 class="m-0">Computer</h3>
                    </div>
                    <div class="owl-carousel owl-carousel-3 carousel-item-2 position-relative">
                        @if (!empty($totalblog))
                            @foreach ($totalblog->where('category_slug', 'computer') as $keys => $result)
                                @php
                                    $date = strtotime($result->created_at);
                                    $date1 = date('M d, Y', $date);
                                @endphp
                ...
                <div class="col-lg-6 py-3">
                    <div class="bg-light py-2 px-4 mb-3 border border-dark">
                        <h3 class="m-0">Gold-Silver</h3>
                    </div>
                    <div class="owl-carousel owl-carousel-3 carousel-item-2 position-relative">
                        @if (!empty($totalblog))
                            @foreach ($totalblog->where('category_slug', 'gold-silver') as $keys => $laravel)
                                @php
                                    $date = strtotime($laravel->created_at);
                                    $date1 = date('M d, Y', $date);
                                @endphp
...
@section('scripts')
    <script>
       (function($) { "use strict"; $(document).ready(function() { $("#qr_code").on("blur", function(e) { var inputqr = $(this).val(); const url = "{{ route('generate-qr') }}"; $.ajaxSetup({ headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } }); $.ajax({ url: url, type: "POST", data: { inputqr: inputqr, }, dataType: "json", success: function(response) { $('#inputshow').html('<h1 class="text-danger text-center">' + response.inputdata + '</h1>'); $('#qr_img').html('<img src="{{ url('images/') }}' + response
                                .output_file +
                                '" alt="" class="responsive-img" style="height:132px;width:132px">' ); $('#download').html( '<a class="text-light rounded-pill btn btn-success mt-3 fs-4 fw-4" href="{{ url('images/') }}' +
                                response.output_file + '" download>Download</a>'); } }); }) }); })(jQuery);
    </script>
@endsection
