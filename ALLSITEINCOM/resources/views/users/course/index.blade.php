                    ...
                    <div class="pb-3">
                        ...
                        <div class="bg-light p-4 mb-3">
                            <form action="{{ route('user.comment.store') }}" method="POST"
                                ...
                                @csrf
                                <input type="hidden" name="category" value="course" required>
                            ...
                            @if (!empty($categorys))
                                @foreach ($categorys as $keys => $category)
                                    <a href="{{ asset('user' . '/' . strtolower($category->category_name)) }}"
                                        class="btn btn-sm btn-outline-secondary m-1 {{ $category->category_name == 'Course' ? 'bg-danger text-light' : '' }}">
    ...
    <script>
        $(document).ready(function() {
            $("#searchbar").on("keyup", function() {
                ...
                const url = "{{ route('user.course-search') }}";
                $.ajaxSetup({
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.courselist != '') {
                            $.each(response.courselist, function(key, blog) {
