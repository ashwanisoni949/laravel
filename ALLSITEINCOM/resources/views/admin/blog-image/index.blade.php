...
@section('admin-content')
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Blog Image</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="#">Home</a></li>
                            <li class="breadcrumb-item active">Blog Image</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
                            ...
                            <div class="card rounded shadow pb-1">
                                <div class=" border-0 quotation_form">
                                    <div
                                        ...
                                        <h5 class="mb-0">Blog Image List</h5>
                                        <button class="btn btn-md btn-primary position_btn" id="add_modal"><i
                                                data-feather="plus" class="lead_icon mg-r-5"></i>Add Blog Image</button></a>
                                    </div>
                                </div>
                                <div class="p-4 mt-1">
                                        ...
                                        <table class="table table-center bg-white mb-0 make_datatable" id="tablegg">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class=" text-center col-sm-7">Images</th>
                                            ...
                                            <tbody>
                                                <!-- Start -->
                                                @if (!empty($blogImage))
                                                    @foreach ($blogImage as $key => $images)
                                                        @php
                                                        ...
                                                        <tr>
                                                            <td class="">{{ $key + 1 }}</td>
                                                            <td class="text-center">{{ $images->name }}</td>
                                                            <td class="text-center">{{ $images->category_id }}</td>
                                                            <td class="text-center">
                                                                @foreach ($image as  $img)
                                                                    <img height="70px" width="90px"  src="{{ asset('/blog-image/' . $img) }}"
                                                                        alt="multiple image"
                                                                        class="w-20 h-20 border border-blue-600 ">
                                                                        {{-- <input type="hidden" value="{{ asset('/blog-image/' . $img) }}" class="myInput{{ $count++ }}"> --}}
                                                                        <input type="hidden" value="{{asset('/blog-image/' . $img) }}" id="imgcount">
                                                                    <button onclick="myFunction(this,)">Copy path</button>
                                                                    <br>
                                                                @endforeach
                                                            </td>
                                                            <td class="text-center">
                                                                <div class="form-check form-switch">
                                                                    <input data-id="{{ $images->id }}"
                                                                        class="form-check-input toggle-class"
                                                                        type="checkbox" data-toggle="toggle"
                                                                        data-on="Active" data-off="InActive"
                                                                        {{ $images->status ? 'checked' : '' }}>
                                                                </div>
                                                            </td>
                                                            <td class="text-center">
                                                                <a href="{{ route('user.comment.edit', $images->id) }}"
                                                                    class="btn btn-primary btn-xs btn-icon table_btn"><i
                                                                        class="fas fa-edit"></i></a>
                                                                <button type="button" id="delete_btn"
                                                                    value="{{ $images->id }}" class="btn btn-danger"><i
        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header border-bottom p-3">
                    <h5 class="modal-title" id="exampleModalLabel">Add Blog Image</h5>
                    <button type="button" class="btn btn-icon btn-close" data-bs-dismiss="modal" id="close-modal"><i
                            class="uil uil-times fs-4 text-dark"></i></button>
                </div>
                <div class="modal-body ">

                    <form method="POST" action="{{ route('admin.blog-image.store') }}" enctype="multipart/form-data"
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="form-icon position-relative">
                                        <input name="images[]" id="images" type="file" class="form-control"
                                            placeholder="Upload Image Name" value="{{ old('images') }}" multiple
                                            required>
                                        <span style="color:red;">
                                            @error('images')
                                                {{ $message }}
    ...
    <script>
        function myFunction(path) {
            ...
            var imgfound = $(path).prev().val();
            console.log(imgfound);
              // Copy the text inside the text field
            navigator.clipboard.writeText(imgfound);
